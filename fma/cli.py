...
from .organizer import apply_moves, plan_moves, scan_directory
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.command == "scan":
        results = scan_directory(config.source_dir)
        print(json.dumps([asdict(x) for x in results], indent=2))
        return 0

    if args.command == "organize":
        planned = plan_moves(config.source_dir, config.destination_dir, config.rules)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    org_cmd = sub.add_parser("organize", help="Organize files by extension rules")
